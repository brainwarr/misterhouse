...
package Insteon::Thermostat::Adapter;

use strict;
use Insteon::BaseInsteon;

@Insteon::Thermostat::Adapter::ISA = ('Insteon::BaseDevice', 'Insteon::BaseObject');
...
sub mode {
    my ( $self, $state ) = @_;
    $state = lc($state);
    debug_log( 1, "$::Time_Date: -> Mode $state");
    ...
    if ( $state eq 'off' ) {
        ...
        &::print_log('[', __PACKAGE__, "] Invalid Mode state: $state\n");
        return ();
    }

    my $message = new Insteon::InsteonMessage('insteon_send', $self, 
                        'thermostat_control', $mode );
    $self->_send_cmd($message);
}
...
sub fan {
    my ( $self, $state ) = @_;
    $state = lc($state);
    debug_log( 1, "$::Time_Date: Fan ->  $state" )
        unless $main::config_parms{no_log} =~ /Insteon_Thermostat/;
    my $fan;
    if ( ( $state eq 'on' ) or ( $state eq 'fan_on' ) ) {
        ...
        &::print_log('[', __PACKAGE__, "] Invalid Fan state: $state\n");
        return ();
    }
    my $message = new Insteon::InsteonMessage('insteon_send', $self, 
                        'thermostat_control', $fan );
    $self->_send_cmd($message);
}

sub cool_setpoint {
    my ( $self, $temp ) = @_;
    debug_log( 1, "$::Time_Date: Cool setpoint ->  $state" )
        unless $main::config_parms{no_log} =~ /Insteon_Thermostat/;
    if ( $temp !~ /^\d+$/ ) {
        print
            "$::Time_Date: [Insteon_Thermostat] ERROR: cool_setpoint $temp not numeric\n";
        return;
    }
...
sub heat_setpoint {
    my ( $self, $temp ) = @_;
    debug_log( 1, "Heat setpoint ->  $state" )
        unless $main::config_parms{no_log} =~ /Insteon_Thermostat/;
    if ( $temp !~ /^\d+$/ ) {
        debug_log( 1, "ERROR: heat_setpoint $temp not numeric");
        return;
    }
...
sub poll_temp {
    ...
    my $subcmd = '00';
    my $message = new Insteon::InsteonMessage('insteon_send', $self, 
                        'thermostat_get_zone_temp', $subcmd );
    $self->_send_cmd($message);
    return;
}
...
sub poll_setpoint {
    ...
    my $subcmd = '20';
    my $message = new Insteon::InsteonMessage('insteon_send', $self, 
                        'thermostat_get_zone_setpoint', $subcmd );
    $self->_send_cmd($message);
    return;
}
...
sub _is_info_request {
    ...
    my $is_info_request = (
               $cmd eq 'thermostat_get_zone_temp'
            or $cmd eq 'thermostat_get_zone_setpoint'
            or $cmd eq 'thermostat_get_zone_humidity'
            or $cmd eq 'thermostat_get_mode'
            or $cmd eq 'thermostat_get_temp'
    ) ? 1 : 0;
    if ($is_info_request) {
        my $val = $msg{extra};
        debug_log( 2, "Processing data for $cmd with value: $val");
        if (   $cmd eq 'thermostat_get_temp'
            or $cmd eq 'thermostat_get_zone_temp' )
        {
...
# by Insteon_Link.
  ...
  # signal receipt of message to the command stack in case commands are queued
                $self->_process_command_stack(%msg);
                &::print_log(
                    "[Insteon_Thermostat] received command/state (awaiting) acknowledge from "
                        . $self->{object_name}
                        . ": $pending_cmd and data: $msg{extra}" )
  ...
  # signal receipt of message to the command stack in case commands are queued
            $self->_process_command_stack(%msg);
            &::print_log(
                "[Insteon_Thermostat] received command/state acknowledge from "
                    . $self->{object_name} . ": "
        ...
        if ( $$self{awaiting_ack} ) {
            &::print_log(
                "[Insteon_Thermostat] WARN!! encountered a nack message ("
        ...
        &::print_log(
            "[Insteon_Thermostat] device category: $msg{devcat} received for "
                . $self->{object_name} );

#stop ping timer now that we have a devcat; possibly may want to change this behavior to allow recurring pings
        $$self{ping_timer}->stop();
    }
    elsif ($msg{command} eq 'thermostat_get_zone_setpoint'
        && $$self{m_pending_setpoint} )
    {
...
#		$self->set($p_state, $p_setby) unless (lc($self->state) eq lc($p_state)) and
        $self->set( $p_state, $self )
            unless ( lc( $self->state ) eq lc($p_state) )
            and ( $msg{type} eq 'cleanup' and $$self{_pending_cleanup} );
        $$self{_pending_cleanup} = 0 if $msg{type} eq 'cleanup';
    }
}

# Getting tired of typing the package part of the print log.
sub debug_log {
   my $debug_level = shift;
   my @msg         = shift;

   if ( $main::Debug{insteon} >= $debug_level ) {
      my $pkg = '[' . __PACKAGE__ .'] ';
      &::print_log($pkg, @msg);
   }
}
